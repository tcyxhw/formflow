...
#### 3.4 新发现筛选标准
只有满足以下任一条件时，才允许把问题写入 `review_evidence.md` 作为新发现：

1. 会影响跨文件或跨层一致性
2. 会影响真实功能、接口契约、类型安全、权限、安全、性能、事务或状态流转
3. 在多个位置重复出现，明显具备复用性
4. 现有规则无法指导当前决策，导致必须猜测或临时约定
5. 未来极可能再次出现，并值得形成规则
...
必须包含：
- 模式：Staged / Branch
- 基准：branch 模式时填写；staged 模式写“当前暂存区”
- 变更文件数
- 总变更行数
- 覆盖方式：完整 / 分组 / 抽查
...
如果本次不是分组 review，也必须保留本节，并明确写“本次未进入分组 review，本节无额外问题”。
